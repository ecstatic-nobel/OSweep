...
def query_list(session):
    ...
    for feed_id in fids:
        ...
        feed_dict["updated_at"]   = parse_resp.get("stamp_pulled", "")
...
def query_feeds(feed_dicts, session):
    ...
    for fd in feed_dicts:
        ...
        for pr in parsed_resp:
            ...
            fdl["stamped_at"] = pr.get("stamp_linked", "")
...
def query_threats(session):
    ...
    for threat_id in tids:
        ...
        threat_dict["wikisummary"] = parse_resp.get("wikisummary", "")
        threat_dict["wikireference"] = parse_resp.get("wikireference", "")
        threat_dict["retired"] = parse_resp.get("retired", "")
        threat_dict["stamp_added"] = parse_resp.get("stamp_added", "")
        threat_dict["stamp_updated"] = parse_resp.get("stamp_updated", "")
        threat_dict["stamp_seen"] = parse_resp.get("stamp_seen", "")
        threat_dict["stamp_retired"] = parse_resp.get("stamp_retired", "")
        threat_dict["updated_last_domain"] = parse_resp.get("updated_last_domain", "")
        threat_dict["othernames"] = "\n".join(parse_resp.get("othernames", ""))
        threat_dict["techniques"] = "\n".join(parse_resp.get("techniques", ""))
        threat_dict["news"] = "\n".join([x.get("link", "") for x in parse_resp["news"]])
...
def write_file(feeds, threats, feed_path, threat_path):
    ...
    with open(feed_path, "w") as ffile, open(threat_path, "w") as tfile:
        fkeys   = feeds[0].keys()
        fheader = ",".join(fkeys)
        ffile.write("{}\n".format(fheader))

        for feed in feeds:
            feed_string = ",".join(feed.values())
            ffile.write("{}\n".format(feed_string.encode("UTF-8")))

        tkeys   = threats[0].keys()
        theader = ",".join(tkeys)
        tfile.write("{}\n".format(theader))

        for threat in threats:
            threat_string = ",".join(threat.values())
            tfile.write("{}\n".format(threat_string.encode("UTF-8")))
    return
...
if __name__ == "__main__":
    if sys.argv[1].lower() == "feed":
        ...
        feed_path      = "{}/pulsedive_feed.csv".format(lookup_path)
        threat_path    = "{}/pulsedive_threat.csv".format(lookup_path)
        write_file(feeds, threats, feed_path, threat_path)
        exit(0)
